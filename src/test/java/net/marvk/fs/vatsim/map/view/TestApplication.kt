...
class PreferencesFxContext() {
    ...
    fun category(name: String, block: CategoryContext.() -> Unit) {
        categories += CategoryContext(name).apply(block).let {
            when (it.visibilityProperty) {
                null -> Category.of(it.name, it.icon, *it.groups.toTypedArray())
                else -> Category.of(it.name, it.icon, it.visibilityProperty, *it.groups.toTypedArray())
            }.subCategories(*it.subCategories.toTypedArray())
        }
    }
}
...
class CategoryContext(val name: String) {
    ...
    fun category(name: String, block: CategoryContext.() -> Unit) {
        subCategories += CategoryContext(name).apply(block).let {
            when (it.visibilityProperty) {
                null -> Category.of(it.name, it.icon, *it.groups.toTypedArray())
                else -> Category.of(it.name, it.icon, it.visibilityProperty, *it.groups.toTypedArray())
            }.subCategories(*it.subCategories.toTypedArray())
...
class GroupContext(val name: String?) {
    ...
    fun doubleSpinner(min: Double, max: Double, amountToStepBy: Double, doubleProperty: DoubleProperty, block: SettingContext.() -> Unit = {}) {
        setting(Spinner<Double>(min, max, min, amountToStepBy).also { /* TODO marku 22-10-14: bind to value */ }, doubleProperty) { block() }
    }

    fun intSpinner(min: Int, max: Int, amountToStepBy: Int, integerProperty: IntegerProperty, block: SettingContext.() -> Unit = {}) {
        setting(Spinner<Int>(min, max, min, amountToStepBy).also { /* TODO marku 22-10-14: bind to value */ }, integerProperty) { block() }
    }
    ...
    fun <T> combobox(options: ObservableList<T>, objectProperty: ObjectProperty<T>, block: SettingContext.() -> Unit = {}) {
        ...
        setting(ComboBox(options).also { /* TODO marku 22-10-14: bind to value */ }, objectProperty) { block() }
    }
        ...
        SettingContext().apply(block).let { context ->
            settings +=
                listOfNotNull(context.textBefore?.let(::Label), node, context.textAfter?.let(::Label))
                    ...
                    .let {
                        VatprismSetting(
                            (context.hiddenSearchableText + listOfNotNull(context.textAfter, context.textBefore, context.textExplanation)).joinToString(),
                            it,
                            observableValues.firstOrNull()
                        )
                    }

            settings += observableValues.drop(1).map { it.asInvisibleSetting() }
        }
    }

    private fun Property<*>.asInvisibleSetting() = VatprismSetting("", Region(), this).invisible()
}
...
private fun PreferencesFx.modifyPreferencesFxStyle() {
    view.parent.stylesheets.addAll(
